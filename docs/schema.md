...
```json
{
  ...
  "name_en": "string",
  "description": "string",
  "specRef": "string, 예: 'TS 38.213 §7.1.1' (base 레이어만 필수)",
...
| `causes` / `explained_by` | 응용 레이어 증상이 기반 레이어 메커니즘으로 설명됨, 또는 증상 간 인과 | Symptom → (Parameter/Formula/Procedure/Symptom) |
| `mitigated_by` | 증상이 특정 메커니즘으로 완화됨 | Symptom → (Procedure/Parameter) |
